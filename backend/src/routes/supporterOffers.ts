...
type PopulatedUser = { _id: unknown; username: string; level: number; points: number; avatar: string }
...
const formatOffer = (o: any) => ({
  ...
  assignedTo: o.assignedTo
    ? { id: String((o.assignedTo as any)._id), username: (o.assignedTo as any).username }
    : null,
...
router.post('/', auth, async (req: AuthRequest, res: Response) => {
  try {
    const { title, description, categories, location, difficulty, durationMinutes } = req.body
...
router.put('/:id/assign', auth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (String(offer.createdBy) === req.userId) {
      res.status(400).json({ message: 'Eigene Angebote können nicht angenommen werden' })
      return
    }
    if (offer.assignedTo) {
    ...
    offer.acceptMessage = typeof req.body.message === 'string' ? req.body.message.trim() : ''
    await offer.save()
...
router.put('/:id/done', auth, async (req: AuthRequest, res: Response) => {
  try {
    ...
    const pointsEarned = offer.pointValue ?? 30
    const user = await User.findById(req.userId)
    if (user) {
      user.points += pointsEarned
      user.level   = calcLevel(user.points)
      await user.save()
      res.json({ message: 'Hilfsangebot abgeschlossen', pointsEarned, newPoints: user.points })
      return
    }
